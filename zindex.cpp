...
#define MAX_QUEUE_SIZE 32768ULL
...
#define MAX_CHUNK_UNCOMPRESSED (2048 * 1024ULL * 1024ULL)

#include <unicode/uchar.h>
#include <unicode/ustring.h>
#include <unicode/utypes.h>
static UChar32 unicode_tolower(UChar32 c) {
  return u_tolower(c); // ICU's actual signature without error parameter
}
/*****************************************************************************
 * SIMD / NEON utilities
...
static uint8_t *base64_decode_neon(const char *encoded, size_t encoded_len,
  ...
  if (encoded_len > 0 && encoded[encoded_len - 1] == '=')
    padding++;
  ...
  if (domain_copy) {
    ...
    if (part_count > 2) {
      ...
      snprintf(main_domain, sizeof(main_domain), "%s.%s",
               domain_parts[part_count - 2], domain_parts[part_count - 1]);
  ...
  if (path) {
    ...
    if (path_copy) {
      ...
      while (segment) {
        ...
        if (segment_idx == 0) {
          char combined[512] = {0};
          snprintf(combined, sizeof(combined), "%s/%s", netloc, segment);
  ...
  *at_sign = '\0';
  ...
  if (domain_copy) {
    ...
    for (int i = 1; i < part_count; i++) {
      ...
      if (strlen(subdomain) >= 4 && bloom_filter_might_contain(bf, subdomain)) {
        result = true;
        break;
      }
    }
...
static bool bloom_filter_might_contain_full(const BloomFilterC *bf,
  ...
  // Quick check in the original case.
  ...
  for (size_t i = 0; i < pattern_len; i++) {
    lowercase_pattern[i] = unicode_tolower((unsigned char)pattern[i]);
  }
  lowercase_pattern[pattern_len] = '\0';

  if (bloom_filter_might_contain(bf, lowercase_pattern)) {
  ...
  char *token_copy = strdup(pattern);
  if (!token_copy) {
    free(lowercase_pattern);
    return true;
  }
  ...
  if (strncmp(pattern, "http://", 7) == 0 ||
    ...
    // Use a fixed buffer for lowercasing tokens (up to 255 characters).
    char token_lower[256];
    while (token) {
      ...
      if (tlen >= 4 && tlen <= 128) {
        ...
        if (tlen < sizeof(token_lower)) {
          for (size_t i = 0; i < tlen; i++) {
            token_lower[i] = unicode_tolower((unsigned char)token[i]);
          }
          token_lower[tlen] = '\0';
          if (bloom_filter_might_contain(bf, token_lower)) {
            result = true;
            break;
          }
        } else {
          // Fallback for overly long tokens.
          char *dyn_lower = (char *)malloc(tlen + 1);
          if (dyn_lower) {
            for (size_t i = 0; i < tlen; i++) {
              dyn_lower[i] = unicode_tolower((unsigned char)token[i]);
            }
            dyn_lower[tlen] = '\0';
            if (bloom_filter_might_contain(bf, dyn_lower)) {
              free(dyn_lower);
              result = true;
              break;
            }
      ...
      if (!bloom_filter_result.error()) {
        ...
        if (!size_result.error() && !num_hashes_result.error() &&
          ...
          if (!bit_array_b64.empty()) {
            ...
            uint8_t *decoded = base64_decode_neon(b64str.c_str(), encoded_len,
                                                  NULL, 0, &decoded_len);
...
static bool boyer_moore_found_neon(const char *text, size_t textLen,
  ...
  while (i <= textLen - patLen) {
    if (patLen >= 16) {
      ...
      if (!neon_compare_aarch64(text + i + tailOffset, pattern + tailOffset,
                                16)) {
  ...
  for (int i = 0; extensions[i] != NULL; i++) {
    ...
    if (extPos) {
      ...
      while (filenameStart > outRes->preview &&
             !(*(filenameStart - 1) == ' ' || *(filenameStart - 1) == '/' ||
               *(filenameStart - 1) == '\\' || *(filenameStart - 1) == ':')) {
        filenameStart--;
      }
...
static void search_in_buffer(PrintQueue *pqueue, const char *filename,
  ...
  while (i + patLen <= textLen && match_count < MAX_MATCHES_PER_CHUNK) {
    ...
    if (matched) {
      ...
      unsigned char c =
          (i + patLen - 1 < textLen) ? (unsigned char)text[i + patLen - 1] : 0;
...
static void full_decompress_and_search(PrintQueue *pqueue, const char *zstFile,
  ...
  if (!ZSTD_isError(zres) && zout.pos > 0) {
    search_in_buffer(pqueue, zstFile, (const char *)outBuf, zout.pos, pattern,
                     patLen, c->uncompressed_start);
  }
...
static void *worker_thread(void *arg) {
  ...
  for (int i = W->start; i < W->end && i < W->fileCount; i++) {
    ...
    search_indexed_file(W->pqueue, W->zstFiles[i], W->idxFiles[i], W->pattern,
                        W->patLen);
  }
...
int main(int argc, char **argv) {
    ...
    if (fnmatch("batch_[0-9][0-9][0-9][0-9][0-9].tar.zst", de->d_name, 0) ==
      ...
      if (stat(idxName, &st) == 0) {
        if (count >= capacity) {
          capacity *= 2;
          char **newZstFiles =
              (char **)realloc(zstFiles, capacity * sizeof(*zstFiles));
          char **newIdxFiles =
              (char **)realloc(idxFiles, capacity * sizeof(*idxFiles));
  ...
  for (int i = 0; i < threadCount; i++) {
    ...
    if (pthread_create(&wargs[i].tid, NULL, worker_thread, &wargs[i]) != 0) {
